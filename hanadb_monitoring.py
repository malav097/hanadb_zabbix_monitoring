...
def main():
    ...
    if sys.argv[1] == '--disks':
        if sys.argv[2] == 'lld':
            ...
            path = "'" + sys.argv[2] + "'"
            m_type = sys.argv[3]
            querystr = query_processor().query_builder(sys.argv[1], path, m_type)
        ...
        service = "'" + sys.argv[2] + "'"
        m_type = sys.argv[3]
        querystr = query_processor().query_builder(sys.argv[1], service, m_type)
        data = query.Exec(querystr)
        value = query_processor().switcher('general', data)
        print(value)
        ...
        if sys.argv[2] == 'lld':
            ...
            schema = "'" + sys.argv[2] + "'"
            m_type = sys.argv[3]
            querystr = query_processor().query_builder(sys.argv[1], schema, m_type)
            data = query.Exec(querystr)
            value = query_processor().switcher('general', data)
            print(value)            
